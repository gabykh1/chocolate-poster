...
# transferring the numbers, to coordinates
cow_body = [x for x in zip(FIRST[::2], FIRST[1::2])]
cow_face = [x for x in zip(SECOND[::2], SECOND[1::2])]
calf_body = [x for x in zip(map(lambda x: x/2 + 200, FIRST[::2]), map(lambda x: x/2 + 200, FIRST[1::2]))]
calf_face = [x for x in zip(map(lambda x: x/2 + 200, SECOND[::2]), map(lambda x: x/2 + 200, SECOND[1::2]))]

# opening the background we will work on
chocolate_background = Image.open('chocolate.jpg')

# drawing on the chocolate template by connecting the coordinates
draw = ImageDraw.Draw(chocolate_background)
draw.line(cow_body, fill="black", width=2)
draw.line(cow_face, fill="black", width=2)

# filling the shapes with color
img1 = chocolate_background.convert("RGB")

ImageDraw.floodfill(chocolate_background,xy=(255,255), value=(120,74,66), border=ImageColor.getrgb("black"),thresh=0)
ImageDraw.floodfill(chocolate_background, xy=(160,150), value=(212,199,188), border=ImageColor.getrgb("black"), thresh=0)

# creating the same for the calf
draw.line(calf_body, fill="black", width=2)
draw.line(calf_face, fill="black", width=2)

ImageDraw.floodfill(chocolate_background,xy=(267,280), value=(212,199,188), border=ImageColor.getrgb("black"),thresh=0)
ImageDraw.floodfill(chocolate_background, xy=(365,310), value=(120,74,66), border=ImageColor.getrgb("black"), thresh=0)
ImageDraw.floodfill(chocolate_background, xy=(290,328), value=(120,74,66), border=ImageColor.getrgb("black"), thresh=0)
ImageDraw.floodfill(chocolate_background, xy=(270, 300), value=(255,204,204), border=ImageColor.getrgb("black"), thresh=0)


# select one coordinate from every horn
horns = [(105, 126), (165, 121), (252, 263), (283, 259), (352, 286)]

# painting all the horns with black color
for xy in horns: ImageDraw.floodfill(chocolate_background, xy=xy, value=ImageColor.getrgb("black"), border=ImageColor.getrgb("black"),thresh=0)

chocolate_background.show()
chocolate_background.save("chocolate cow.png")
